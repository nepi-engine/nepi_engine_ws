...
2026-09 — RUI build cut 63s to 15s by three env-gated levers, all measured on device — `build_nepi_rui.sh` exports three overridable defaults before `npm run build`, and `src/nepi_rui/.../config-overrides.js` implements them with no new dependencies. Measured on the Jetson: `GENERATE_SOURCEMAP=false` (also drops ~7MB of never-consumed source maps from the deploy) plus babel-loader `cacheDirectory` take 63s to 32s, the babel cache alone being worth 18s (50s cold vs 32s warm); `RUI_SKIP_LINT=1` is worth 8s; `RUI_NO_COMPRESS=1` is worth 8s and costs ~13KB gzipped (292 to 305KB). Each default is set as `${VAR:-default}`, so `RUI_SKIP_LINT=0 ruibld` restores the lint pass (37 warnings), `RUI_NO_COMPRESS=0 ruibld` restores full minification, and `GENERATE_SOURCEMAP=true ruibld` restores browser-debuggable JS. Lint is off by default deliberately: eslint-loader only ever warns here (nothing sets CI=true) so skipping it changes no output, but it is the only eslint signal in the repo — `npm run lint` is prettier, i.e. formatting, not linting.
