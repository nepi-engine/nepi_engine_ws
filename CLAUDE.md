...
2026-09 — RUI build cut 63s to ~26s by env-gated levers, all measured on device, with source maps deliberately kept — `build_nepi_rui.sh` exports three overridable defaults before `npm run build`, and `src/nepi_rui/.../config-overrides.js` implements them with no new dependencies. Measured on the Jetson: `GENERATE_SOURCEMAP=false` (which also drops ~7MB from the deploy) plus babel-loader `cacheDirectory` take 63s to 32s, the babel cache alone being worth 18s (50s cold vs 32s warm); `RUI_SKIP_LINT=1` is worth 8s; `RUI_NO_COMPRESS=1` is worth 8s and costs ~13KB gzipped (292 to 305KB). All three on reached 15s.

Source maps were then turned back ON by default (2026-09-17) and the ~11s given back, because without them a browser stack trace lands in minified `main.<hash>.js` and a runtime RUI error cannot be traced to a source file — an undebuggable UI is not worth 11s of build time. The other two levers stay on: they cost only bundle size and a warning pass, not diagnosability. Each default is `${VAR:-default}`, so `GENERATE_SOURCEMAP=false ruibld` buys the 11s back for a throwaway build, `RUI_SKIP_LINT=0 ruibld` restores the lint pass (37 warnings), and `RUI_NO_COMPRESS=0 ruibld` restores full minification. Note `config-overrides.js` derives uglify's own `sourceMap` from the same env var, so the two never disagree. Lint is off by default deliberately: eslint-loader only ever warns here (nothing sets CI=true) so skipping it changes no output, but it is the only eslint signal in the repo — `npm run lint` is prettier, i.e. formatting, not linting.

Three findings worth not rediscovering. The script preamble (rsync, `chmod -R`, app scan, sed codegen) is only 3s of the total, so it is not a target; `build_nepi_rui.sh` now prints `RUI timing: setup+rsync Xs | npm build Ys | total Zs` to keep that visible. Parallel minification buys nothing here because the RUI compiles to a single bundle (`main.<hash>.js`) — `uglifyjs-webpack-plugin` with `parallel`/`cache` was implemented, measured at zero gain across two passes, and removed; earlier apparent gains came from rebuilding byte-identical bundles, which never happens once a source file changes. Do not re-add that dependency: webpack 3.8.1 already depends on `uglifyjs-webpack-plugin` ^0.4.6, which hoists to the top level on any node_modules installed without it, and 0.4.6 has no `uglifyOptions`/`parallel`/`cache` keys at all — it silently ignores the whole options object and minifies with defaults, dropping the `ascii_only` and `comparisons:false` workarounds react-scripts depends on, from a build that reports success. `bench_rui_build.sh` at the repo root re-runs the whole matrix on device in one pass and restores the default build when it finishes.
